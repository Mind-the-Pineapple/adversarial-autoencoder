...
experiment_dir = output_dir / 'unsupervised_aae_gaussian_posterior_convolutional'
...
def make_encoder_model(z_size):
    ...
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.LeakyReLU(0.2)(x)
    mean = tf.keras.layers.Conv2D(filters=z_size, kernel_size=3, strides=2, padding='same')(x)
    stddev = tf.keras.layers.Conv2D(filters=z_size, kernel_size=3, strides=2, padding='same', activation='softplus')(x)
...
# -------------------------------------------------------------------------------------------------------------
# Define optimizers
ae_optimizer = tf.keras.optimizers.Adam(lr=0.0001)
dc_optimizer = tf.keras.optimizers.Adam(lr=0.0001)
gen_optimizer = tf.keras.optimizers.Adam(lr=0.0001)
