...
experiment_dir = output_dir / 'unsupervised_aae_gaussian_posterior'
...
def make_decoder_model():
    ...
    x = tf.keras.layers.LeakyReLU()(x)
    x = tf.keras.layers.Dropout(0.5)(x)
    reconstruction = tf.keras.layers.Dense(image_size)(x)
...
def make_discriminator_model():
    encoded = tf.keras.Input(shape=(z_dim,))
    x = tf.keras.layers.Dense(h_dim / 4)(encoded)
    x = tf.keras.layers.LeakyReLU()(x)
    x = tf.keras.layers.Dense(h_dim / 4)(x)
    x = tf.keras.layers.LeakyReLU()(x)
